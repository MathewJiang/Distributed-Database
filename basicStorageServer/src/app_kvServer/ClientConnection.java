...
public class ClientConnection implements Runnable {
	...
	/****************************************************************************
	 * willMigrate
	 * Will the current server do migration
	 * !Assume: only one server needs migration at a time
	 * 
	 * @return	true if the callingServer will do migration
	 * 			false if the callingServer will not do migration 
	 * 
	 * @throws 	Exception		if failed to find range
	 * !!!!!FIXME: need to change to "private"
	 * !!!!!FIXME: does not consider server collision case
	 * 			   e.g. newly added server MD5 value coincides with existing server
	 * 
	 ****************************************************************************/
	public boolean willMigrate(InfraMetadata oldMetaData, InfraMetadata newMetaData) throws Exception {
		...
		BigInteger newNodeMD5 = MD5.getMD5(new String (newNode.host + ":" + newNode.port));
		...
		try {
			...
			ServiceLocation sl = new ServiceLocation(callingServer.getName(), callingServer.getHostname(), callingServer.getPort());
	 ...
	 * @throws IOException 
	 ****************************************************************************/
	private void migrationTo(InfraMetadata oldMetaData, InfraMetadata newMetaData) throws IOException {
	...
	/****************************************************************************
	 * migrationFrom
	 * Received the data migrated from the other server
	 * !Assume: server invoking this method must the newly added server
	 * 
	 ****************************************************************************/
	private void migrationFrom(InfraMetadata newMetaData, InfraMetadata oldMetaData) {
	...
	/****************************************************************************
	 * handleGET
	 * get the corresponding value associated with key
	 * @param	key
	 * 
	...
	public static void main(String[] args) {
		ClientConnection conn = new ClientConnection(new Socket(), new KVServer(50006, 5, "LRU"));
		InfraMetadata newMetaData = null;
		InfraMetadata oldMetaData = null;
		try {
			newMetaData = InfraMetadata.fromConfigFile("/nfs/ug/homes-3/j/jiangz32/ECE419/ECE419-Distributed-Systems/basicStorageServer/newMetaData.properties");
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			oldMetaData = InfraMetadata.fromConfigFile("/nfs/ug/homes-3/j/jiangz32/ECE419/ECE419-Distributed-Systems/basicStorageServer/oldMetaData.properties");
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		for (ServiceLocation s : sls) {
			try {
				...
				System.out.println(s.serviceName + "'s Hashrange: " + hashrang[0] + "~" + hashrang[1]);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		System.out.println("server4's MD5 value: " + MD5.getMD5String("127.0.0.1:50003"));
